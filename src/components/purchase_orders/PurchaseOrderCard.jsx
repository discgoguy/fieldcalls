        ...
        <Card onClick={onClick} className="cursor-pointer hover:shadow-lg transition-shadow duration-200 flex flex-col h-full">
            ...
            <CardFooter className="text-xs text-gray-500 bg-gray-50/50 p-3">
                <div className="flex flex-col gap-0.5 w-full">
                    <span>Ordered: {format(purchaseOrder.order_date, 'MMM d, yyyy')}</span>
                    {purchaseOrder.status === 'Complete' && purchaseOrder.date_completed && (
                        <span className="text-green-600 font-medium">
                            Completed: {format(purchaseOrder.date_completed, 'MMM d, yyyy')}
                        </span>
                    )}
                </div>
            </CardFooter>
