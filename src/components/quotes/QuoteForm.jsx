...
export default function QuoteForm({ customers, parts, categories, onSubmit, initialQuote, initialItems }) {
    ...
    const handleItemChange = (index, field, value) => {
        ...
        if (field === 'quantity' || field === 'unit_price') {
            // Allow empty string while typing, convert to number otherwise
        ...
        if (field === 'part_id') {
            ...
            if (part) {
                ...
                newItems[index].unit_price = useNonSA ? (part.nonsa_price || part.sales_price || 0) : (part.sales_price || 0);
            }
        }
            ...
            <div className="space-y-3 p-3 border rounded-lg">
                ...
                {items.map((item, index) => {
                    const filteredParts = parts
                        .filter(p => 
                            (!p.is_obsolete || p.id === item.part_id) &&
                            !p.is_internal &&
                            (item.category === 'all' || p.category === item.category)
                        )
                        .sort((a, b) => (a.part_name || '').localeCompare(b.part_name || ''));

                    return (
                        <div key={index} className="flex flex-col md:flex-row md:items-end gap-2 p-2 bg-slate-50 rounded">
                            <div className="w-full md:w-1/6"><Label>Type</Label><Select value={item.item_type} onValueChange={(val) => handleItemChange(index, 'item_type', val)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="Part">Part</SelectItem><SelectItem value="Service">Service</SelectItem><SelectItem value="Custom">Custom</SelectItem></SelectContent></Select></div>
                            ...
                            {item.item_type === 'Part' ? (
                                <>
                                    ...
                                    <div className="w-full md:flex-1"><Label>Part</Label><Select value={item.part_id} onValueChange={(val) => handleItemChange(index, 'part_id', val)} disabled={filteredParts.length === 0}><SelectTrigger><SelectValue placeholder="Select a part"/></SelectTrigger><SelectContent>{filteredParts.map(p => <SelectItem key={p.id} value={p.id}>{`${p.part_name} (${p.part_number})`}</SelectItem>)}</SelectContent></Select></div>
                                </>
                            ) : (
                                <div className="w-full md:flex-1"><Label>Description</Label><Input value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)}/></div>
                            )}

                            <div className="w-full md:w-24"><Label>Qty</Label><Input type="number" value={item.quantity} onChange={(e) => handleItemChange(index, 'quantity', e.target.value)} onBlur={(e) => !e.target.value && handleItemChange(index, 'quantity', 1)} min="1"/></div>
                            <div className="w-full md:w-32"><Label>Unit Price</Label><Input type="number" step="0.01" value={item.unit_price} onChange={(e) => handleItemChange(index, 'unit_price', e.target.value)} onBlur={(e) => !e.target.value && handleItemChange(index, 'unit_price', 0)} disabled={item.item_type === 'Part'}/></div>
                            <div className="w-full md:w-32"><Label>Total</Label><Input value={`$${((item.quantity || 0) * (item.unit_price || 0)).toFixed(2)}`} disabled className="font-semibold"/></div>
                            <div className="flex justify-end md:block"><Button type="button" variant="ghost" size="icon" onClick={() => removeItem(index)}><Trash2 className="w-4 h-4 text-red-500"/></Button></div>
                        </div>
                    )
                })}
