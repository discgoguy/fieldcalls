...
import { Customer, Machine, Part, Transaction } from '@/api/entities';
...
export default function ServiceHistoryReport() {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            setLoading(true);
            const [customerData, partData, sortSettings] = await Promise.all([
                Customer.list(), 
                Part.list(),
                applySortSettings()
            ]);
            const activeCustomers = (customerData || []).filter(c => !c.inactive);
            const sortedCustomers = sortArray(activeCustomers, sortSettings.customers);
    ...
    const handleCustomerChange = async (customerId) => {
        ...
        setReportData(null);
        if (customerId) {
            const customerMachines = await Machine.filter({ customer_id: customerId });
    ...
    const generateReport = async () => {
        ...
        const filter = { 
            ...
            transaction_type: { $in: ['on_site_service', 'in_house_service', 'service_expense', 'parts_order', 'shipping_expense', 'no_charge', 'warranty_replacement', 'service_agreement', 'sales_agreement'] },
            date: {} // Initialize date filter object
        };

        if (dateRange.from) {
            filter.date.$gte = ((dateRange.from) ? format(new Date(dateRange.from), 'yyyy-MM-dd') : '—');
        }
        if (dateRange.to) {
            const toDate = new Date(dateRange.to);
            toDate.setDate(toDate.getDate() + 1); // Add one day to include the entire 'to' day
            filter.date.$lt = ((toDate) ? format(new Date(toDate), 'yyyy-MM-dd') : '—'); // Use $lt for less than the start of the next day
        }

        const transactions = await Transaction.filter(filter, '-date');

        const partMap = parts.reduce((acc, p) => ({ ...acc, [p.id]: p }), {});

        const relevantTransactions = transactions.filter(t => {
            if (selectedMachineId === 'all') {
                return !t.machine_id || machineIds.includes(t.machine_id);
            }
            return t.machine_id && machineIds.includes(t.machine_id);
        });
            ...
            <div className="print-area mt-6 border p-4 sm:p-8 rounded-lg bg-white">
                <div className="flex justify-between items-start mb-8">
                    ...
                    <div className="text-right">
                        <p className="text-sm">Generated on: {((generatedAt) ? format(new Date(generatedAt), 'PPP') : '—')}</p>
                    ...
                    <div className="flex-1 w-full">
                        ...
                        <Select onValueChange={setSelectedMachineId} value={selectedMachineId} disabled={!selectedCustomerId}>
                            ...
                            <SelectContent>
                                ...
                                {machines.sort((a, b) => a.model.localeCompare(b.model)).map(m => <SelectItem key={m.id} value={m.id}>{m.model} (S/N: {m.serial_number})</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                    <div className="flex-1 w-full">
