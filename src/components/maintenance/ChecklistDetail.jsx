...
import { MaintenanceChecklist, MaintenanceChecklistItem } from '@/api/entities';
...
import { Printer, Save, Loader2, Wrench, Trash2, Layers } from 'lucide-react';
import { isOptionSelected, toggleMultiSelectValue, parseMultiSelect } from '@/lib/multiSelectUtils';
...
export default function ChecklistDetail({ checklist, customer, machines, technicians, parts, isOpen, onOpenChange, onUpdate }) {
    ...
    const handleTaskResponse = async (itemId, responseValue, completed) => {
        // Update UI immediately
        setItems(prevItems => prevItems.map(item =>
            item.id === itemId ? { ...item, response_value: responseValue, completed } : item
        ));
        // Auto-save this item immediately in the background
        try {
            await MaintenanceChecklistItem.update(itemId, {
                completed: completed || false,
                response_value: responseValue || '',
                is_completed: completed || false,
            });
        } catch (e) {
            console.error('Auto-save failed for item', itemId, e);
            // Don't show error to user - will be caught on full save
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await MaintenanceChecklist.update(checklist.id, {
                status,
                section_notes: sectionNotes
            });

            // Save all items - individual items are already auto-saved on check
            // This final save catches any that may have failed
            const BATCH = 20;
            for (let i = 0; i < items.length; i += BATCH) {
                await Promise.all(
                    items.slice(i, i + BATCH).map(item =>
                        MaintenanceChecklistItem.update(item.id, {
    ...
    const handleDelete = async () => {
        try {
            for (const item of items) {
                await MaintenanceChecklistItem.delete(item.id);
            }
            await MaintenanceChecklist.delete(checklist.id);
            onUpdate();
        } catch (e) {
    ...
    const groupedItems = machines.reduce((acc, machine) => {
        ...
        const machineItems = items.filter(item => item.machine_id === machine.id).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
                                        ...
                                        return (
                                            <div key={sectionName} className="border rounded-lg p-4 space-y-3">
                                                <h4 className="font-medium text-blue-700 flex items-center gap-2">
                                                    <Layers className="h-4 w-4" />{sectionName}
                                                </h4>
                                                <div className="overflow-x-auto">
                                                    <table className="w-full text-sm border-collapse">
                                                        <thead>
                                                            <tr>
                                                                <th className="text-left p-2 border-b font-medium">Instance</th>
                                                                {taskKeys.map(tk => {
                                                                    const sample = orderedItems.find(i => i.task_key === tk);
                                                                    return <th key={tk} className="text-left p-2 border-b font-medium">{sample?.task_description}</th>;
                                                                })}
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {instanceLabels.map(label => (
                                                                <tr key={label}>
                                                                    <td className="p-2 border-b font-medium text-gray-700 whitespace-nowrap">{label}</td>
                                                                    {taskKeys.map(tk => {
                                                                        const item = cellFor(label, tk);
                                                                        if (!item) return <td key={tk} className="p-2 border-b" />;
                                                                        return (
                                                                            <td key={tk} className="p-2 border-b align-top">
                                                                                ...
                                                                                {item.task_type === 'multiple_choice' && (
                                                                                    <div className="flex flex-wrap gap-x-3 gap-y-1">
                                                                                        {(item.options || []).map(option => (
                                                                                            <label key={option} className="flex items-center gap-1 cursor-pointer whitespace-nowrap">
                                                                                                <Checkbox
                                                                                                    className="h-3.5 w-3.5"
                                                                                                    checked={isOptionSelected(item.response_value, option)}
                                                                                                    onCheckedChange={(checked) => {
                                                                                                        const newValue = toggleMultiSelectValue(item.response_value, option, checked);
                                                                                                        handleTaskResponse(item.id, newValue, parseMultiSelect(newValue).length > 0);
                                                                                                    }}
                                                                                                />
