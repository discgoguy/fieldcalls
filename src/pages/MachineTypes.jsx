...
import { MachineType } from '@/api/entities';
...
export default function MachineTypesPage() {
    ...
    const loadMachineTypes = async () => {
        setLoading(true);
        try {
            const data = await MachineType.list();
            setMachineTypes((data || []).sort((a, b) => a.name.localeCompare(b.name)));
        } catch (e) {
    ...
    const handleSaveMachineType = async (e) => {
        ...
        try {
            ...
            if (editingMachineType) {
                await MachineType.update(editingMachineType.id, newMachineType);
                setSuccessMessage("Machine type successfully updated!");
            } else {
                await MachineType.create(newMachineType);
    ...
    const handleDeleteMachineType = async (machineTypeId) => {
        try {
            await MachineType.delete(machineTypeId);
