...
import { Customer, Part, Technician, Transaction } from '@/api/entities';
      ...
      switch (exportType) {
        case 'transactions': {
          ...
          const [transactions, customers, parts, technicians] = await Promise.all([
            ...
            Customer.list('company_name', 5000),
            Part.list('part_name', 5000),
            Technician.list('full_name', 5000)
          ]);
        ...
        case 'customers': {
          const customers = await Customer.list('company_name', 5000);
          dataToExport = customers.map(({ id, created_date, updated_date, ...rest }) => rest);
          break;
        }
        case 'parts': {
          const parts = await Part.list('part_name', 5000);
          // compatible_machine_types is a jsonb array - when it holds multiple values,
          // the comma-joined result isn't quoted like a real CSV cell, so it gets read
          // as extra columns and shifts everything after it in that row. Dropping it
          // from the export avoids that until/unless we want to fix convertToCSV itself
          // to properly quote array-valued cells.
          dataToExport = parts.map(({ id, created_date, updated_date, compatible_machine_types, ...rest }) => rest);
          break;
        }
        default:
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button id="date" variant={"outline"} className="w-[300px] justify-start text-left font-normal">
                      ...
                      {dateRange?.from ? (
                        dateRange.to ? (
                          <>{((dateRange.from) ? format(new Date(dateRange.from), "LLL dd, y") : '—')} - {((dateRange.to) ? format(new Date(dateRange.to), "LLL dd, y") : '—')}</>
                        ) : ((dateRange.from) ? format(new Date(dateRange.from), "LLL dd, y") : '—')
