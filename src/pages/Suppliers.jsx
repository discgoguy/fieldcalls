...
import { Part, Supplier } from '@/api/entities';
 // Added Part entity import
...
export default function SuppliersPage() {
    ...
    const loadSuppliers = async () => {
        ...
        try {
            ...
            const sortedData = sortArray(data || [], sortSettings.suppliers);
    ...
    const handleSyncSuppliers = async () => {
        ...
        try {
            ...
            const [allParts, currentSuppliers] = await Promise.all([
                Part.list(null, 10000), // Get all parts, assuming 10000 is a sufficient limit
                Supplier.list()
            ]);
            ...
            if (suppliersToAdd.length > 0) {
                ...
                // Assuming Supplier.bulkCreate exists and takes an array of { name: string }
                const newSupplierRecords = suppliersToAdd.map(name => ({ name }));
    ...
    const handleSaveSupplier = async (e) => {
        ...
        try {
            ...
            if (editingSupplier) {
                await Supplier.update(editingSupplier.id, newSupplier);
